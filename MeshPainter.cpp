...
TextureLibrary texture_library = TextureLibrary();
...
void MeshPainter::bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::string &texture_image, const std::string &vshader, const std::string &fshader)
{
    ...
    // 生成纹理对象
    object.texture = texture_library.getTexture(object.texture_image);
...
{
    std::vector<std::string>::iterator found_name = find(mesh_names.begin(), mesh_names.end(), name);
    if (found_name != mesh_names.end())
    {
        return;
    }
...
void MeshPainter::deleteMesh(const std::string &name)
{
    std::vector<std::string>::iterator found_name = find(mesh_names.begin(), mesh_names.end(), name);
    if (found_name == mesh_names.end())
        return;

    int index = distance(mesh_names.begin(), found_name);

    mesh_names.erase(mesh_names.begin() + index);
...
void MeshPainter::drawMesh(TriMesh *mesh, openGLObject &object, Light *light, Camera *camera)
{
    ...
    if (mesh->mesh_name == "centering")
    {
        glm::mat3 _Matrix = glm::mat3(camera->projMatrix);
        glm::mat4 _Matrix4 = glm::mat4(_Matrix);
        glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &_Matrix4[0][0]);
    }
    else
        glUniformMatrix4fv(object.projectionLocation, 1, GL_FALSE, &camera->projMatrix[0][0]);

    if (mesh->isTransparency)
    {
...
void MeshPainter::drawMeshes(Light *light, Camera *camera)
{
    ...
    {
        // glm::mat4 clipMatrix = camera->getProjectionMatrix(false) * camera->getViewMatrix();
...
{
    std::cout << "Loading texture: " << file_name << std::endl;
    // 读取纹理图片，并将其传递给着色器
